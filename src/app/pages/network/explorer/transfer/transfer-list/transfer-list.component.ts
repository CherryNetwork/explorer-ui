...
export class TransferListComponent
  extends PaginatedListComponentBase<pst.Event | pst.AccountEvent>
  implements OnInit
{
  listSize = 100;
  ...
  ): Promise<pst.ListResponse<pst.Event | pst.AccountEvent>> {
  ...
  createNewItemSubscription(
    handleItemFn: (item: pst.Event | pst.AccountEvent) => void
  ): Promise<() => void> {
    if (this.addressControl.value) {
      return this.pa
        .run(this.network)
      ...
      return this.pa
        .run(this.network)
        .polkascan.chain.subscribeNewEvent(this.filters, handleItemFn);
    }
  }
