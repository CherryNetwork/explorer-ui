...
export class HistoricalBalanceComponent
  extends PaginatedListComponentBase<pst.AccountEvent>
  implements OnInit, OnChanges
{
  ...
  constructor(
    ...
    this.pa
      ...
      .rpc.chain.getBlockHash(1)
      .then((hash) => this.blockOne.next(hash));
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.accountId.currentValue !== changes.accountId.previousValue) {
      ...
      typeof this.unsubscribeNewItem === 'function'
  ...
  createGetItemsRequest(
    ...
    if (this.accountId) {
      return this.pa
        .run(this.network)
        .polkascan.chain.getEventsByAccount(
  ...
  createNewItemSubscription(
    handleItemFn: (item: pst.AccountEvent) => void
  ): Promise<() => void> {
    if (this.accountId) {
      return this.pa
        .run(this.network)
        .polkascan.chain.subscribeNewEventByAccount(
          this.accountId?.toHex(),
          this.filters,
          handleItemFn
        );
  ...
  createItemAtBlockOneObservable(): void {
    ...
    this.itemAtBlockOne = this.itemsObservable.pipe(
      ...
      map<pst.AccountEvent[], boolean>(() => {
        // Check that the items list is at its end.
        if (this.pageNext) {
          return false;
        } else if (this.blockLimitOffset && this.blockLimitCount) {
          const blockLimitOffset = Math.max(
            0,
            this.blockLimitOffset - this.blockLimitCount
          );
      ...
      combineLatestWith(this.blockOne.pipe(takeUntil(this.destroyer))),
      switchMap(([atListStart, hash]) => {
        if (atListStart && this.accountId) {
          // The items list is at its end. Check if at genesis there was an AccountInfo.
          const timestampObservable = from(
            this.pa.run(runParams).query.timestamp.now.at(hash)
          ).pipe(
            map((timestamp) => {
          ...
          return (
            from(
              this.pa
                .run(runParams)
                .query.system.account.at(hash, this.accountId)
            ) as Observable<AccountInfo>
          ).pipe(
            combineLatestWith(timestampObservable),
            map<[AccountInfo, number | null], BalancesItem | null>(
                ...
                if (accountInfo && accountInfo.data) {
                  if (
                    ...
                    if (this.balancesPerBlock.has(1) === false) {
                      this.balancesPerBlock.set(
                        1,
                        new BehaviorSubject<HistoricalBalance>({})
                      );
  ...
  createBalancesObservable(): void {
    this.balancesObservable = this.itemsObservable.pipe(
      ...
      map<pst.AccountEvent[], pst.AccountEvent[]>((items) => {
        // Filter out double blocks
        return items.filter(
          (item) =>
            items.find((other) => other.blockNumber === item.blockNumber) ===
            item
        );
      }),
      map<pst.AccountEvent[], BalancesItem[]>((items) => {
        const result = items
          .map((event) => {
            ...
            return {
              event: event,
              balances: this.balancesPerBlock.get(event.blockNumber),
            } as BalancesItem;
          })
      ...
      map<[BalancesItem[], BalancesItem | null], BalancesItem[]>(
        ([balanceItems, itemAtBlockOne]) => {
          if (itemAtBlockOne) {
  ...
  createChartDataObservable(): void {
    ...
    this.chartDataObservable = this.balancesObservable.pipe(
      ...
      combineLatestWith(
        // Check if the account has a balance at the latest block height.
        from(this.pa.run(runParams).rpc.chain.getFinalizedHead()).pipe(
          switchMap((hash) =>
            from(
              this.pa
                .run(runParams)
                .query.system.account.at(
                  hash,
                  this.accountId
              ...
              combineLatestWith(
                from(this.pa.run(runParams).rpc.chain.getHeader(hash)).pipe(
                  map<Header, number>(
                    (header) => header.number.toJSON() as number
                  )
                ),
                from(this.pa.run(runParams).query.timestamp.now.at(hash)).pipe(
                  map<Codec, number>(
                    (timestamp) => timestamp.toJSON() as EpochTimeStamp
                  )
                )
              )
            )
          ),
          map<[AccountInfo, number, number], BalancesItem | null>(
            ([accountInfo, blockNumber, timestamp]) => {
              if (accountInfo && accountInfo.data) {
                if (
                  accountInfo.data.free
                    .add(accountInfo.data.reserved)
      ...
      switchMap<BalancesItem[], Observable<(ChartItem | null)[]>>(
        (bis): Observable<(ChartItem | null)[]> =>
          combineLatest(
            bis.map((bi) =>
              combineLatest([of(bi.event), bi.balances]).pipe(
                filter(([event, balances]) => Boolean(event && balances)),
                map<[pst.AccountEvent, HistoricalBalance], ChartItem | null>(
                  ([event, balances]): ChartItem | null => {
                    if (this.chartItemPerBlock.has(event.blockNumber)) {
                      return this.chartItemPerBlock.get(
                        event.blockNumber
                      ) as ChartItem;
                    }
                    ...
                    if (event && event.blockDatetime && balances) {
                      ...
                      const reserved = this.convertBNforChart(
                        balances.reserved
                      );
                      const locked = this.convertBNforChart(balances.locked);
                      const transferable = this.convertBNforChart(
                            ...
                            total !== null
                              ? parseFloat(
                                  total
                                    .split('.')
                                    .map((b, i) =>
                                      i === 1 ? b.substring(0, 2) : b
                                    )
                                    .join('.')
                                )
                              : null, // Only two decimals in the chart
      ...
      map<(ChartItem | null)[], ChartItem[]>(
        (items) =>
          items
            .filter((i) => i !== null)
            .sort((a, b) => +a!.blockDate - +b!.blockDate) as ChartItem[]
      ),
      combineLatestWith(this.pricing.dailyHistoricPrices),
      map<[ChartItem[], [number, number][] | undefined], Highcharts.Options>(
        ([items, historicPrices]): Highcharts.Options => {
          let pointFormat =
            '<b>{point.x:%Y-%m-%d %H:%M:%S}</b><br>' +
          ...
          if (historicPrices && historicPrices.length > 0) {
            ...
            pointFormat =
              pointFormat +
              '<br>' +
              'Historic value: {point.historicValue} ' +
              this.variables.currency.value;

            if (min !== null && max !== null) {
              // Create a second currency based historic series.
              const historicPricesInScope = historicPrices.filter(
                (p) => p[0] >= min! && p[0] <= max!
              );
              ...
              const valuePerItem = items
                .map((item) => {
                  if (
                    item.historicValue !== undefined &&
                    item.historicValue !== null
                  ) {
                    return [
                      item.blockDate.getTime(),
                      parseFloat(item.historicValue),
                    ];
                  }
                  return null;
                })
  ...
  async getBalanceAtBlock(blockNumber: number): Promise<void> {
    ...
    try {
      ...
      if (systemAccount && systemAccount.data) {
        ...
        balances.total = systemAccount.data.free.add(
          systemAccount.data.reserved
        );
        balances.transferable = systemAccount.data.free.sub(
          systemAccount.data.feeFrozen
        );
    ...
    const [accountData, locks, freeBalance, reservedBalance] = (
      await Promise.allSettled([
        this.pa
          .run(runParams)
          .query.balances.account.at(blockHash, this.accountId),
        this.pa
          .run(runParams)
          .query.balances.locks.at(blockHash, this.accountId),
        this.pa
          .run(runParams)
          .query.balances.freeBalance.at(blockHash, this.accountId),
        this.pa
          .run(runParams)
          .query.balances.reservedBalance.at(blockHash, this.accountId),
      ])
  ...
  convertBNforChart(val: BN | number | undefined | null): string | null {
    if (BN.isBN(val)) {
      ...
      try {
        ...
        const integralPart = stringified
          .substring(0, l - decimals)
          .replace(/^0+\B/, ''); // remove preceding zeros, but allow a value of '0'.
        const decimalPart = stringified
          .substring(l - decimals)
